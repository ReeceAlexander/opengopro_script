...
class WirelessGoPro(GoProBase[WirelessApi], GoProWirelessInterface):
    ...
    async def open(self, timeout: int = 15, retries: int = 5) -> None:
        """Custom open method to initialize BLE without using set_date_time_tz_dst."""
        ...
        try:
            ...
            # # ✅ Step 2: Get API Version (Confirm Compatibility)
            # try:
            #     camera_settings = await self.ble_command.get_camera_settings()
            #     print(f"Camera Settings: {camera_settings}")
            # except Exception as e:
            #     print(f"❌ Failed to get camera settings: {e}")
            #     return

            # # ✅ Step 3: Manually Set Time (32-bit UNIX timestamp)
            # try:
            #     current_time = int(time.time()) & 0xFFFFFFFF  # Ensure 32-bit format
            #     #print(f"Setting GoPro time to: {datetime.utcfromtimestamp(current_time)} UTC")
            #     await self.ble_command.set_date_time(current_time)
            #     print("✅ Date & Time updated successfully!")
            # except Exception as e:
            #     print(f"❌ Failed to set time: {e}")

            # # ✅ Step 4: Handle WiFi (if needed)
            # if self._should_enable_wifi:
            #     await self._open_wifi(timeout, retries)
            # else:
            #     print("Turning off the camera's WiFi.")
